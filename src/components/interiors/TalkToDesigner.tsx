    ...
    <div className="fixed bottom-5 right-4 z-40 flex flex-col items-end gap-3 sm:bottom-8 sm:right-8">
      {open ? (
        <div className="w-64 border border-border bg-card p-5 shadow-xl">
          ...
          <p className="mt-2 font-display text-lg text-navy">Talk to a designer</p>
          <a
            href={designerPhoneHref}
            className="mt-4 flex items-center gap-3 border border-border px-3 py-3 text-sm text-navy transition-colors hover:border-gold"
          >
            <Phone className="h-4 w-4 text-teal" aria-hidden="true" />
            <span>
              Call now
              <span className="block text-xs text-muted-foreground">{designerPhone}</span>
            </span>
          </a>
          <Link
            to="/interior-design/consultation"
            onClick={() => setOpen(false)}
            className="mt-3 flex items-center gap-3 bg-navy px-3 py-3 text-sm text-primary-foreground transition-colors hover:bg-teal"
          >
      ...
      <button
        ...
        aria-expanded={open}
        className="flex items-center gap-2 bg-navy px-5 py-3.5 text-sm font-medium text-primary-foreground shadow-lg transition-colors hover:bg-teal"
      >
        {open ? (
