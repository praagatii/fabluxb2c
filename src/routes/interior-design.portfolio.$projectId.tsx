      ...
      <section className="relative">
        ...
        <div className="absolute inset-0 grid items-end pb-10 sm:pb-16">
          <Container>
            ...
            <h1 className="mt-3 max-w-3xl font-display text-3xl leading-tight text-primary-foreground sm:text-5xl">
              {project.title}
            </h1>
          </Container>
        </div>
      ...
      <Section>
        <dl className="grid gap-px border border-border bg-border sm:grid-cols-5">
          {meta.map((item) => (
            <div key={item.label} className="bg-card p-5">
              ...
              <dd className="mt-2 text-sm text-navy">{item.value}</dd>
            </div>
          ))}
        </dl>
        ...
        <div className="mt-12 grid gap-10 lg:grid-cols-[1.3fr_1fr]">
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{project.brief}</p>
            <p className="label-eyebrow mt-8 text-teal">What we did</p>
            <ul className="mt-4 space-y-3">
              {project.work.map((item) => (
                <li key={item} className="border-l-2 border-gold pl-4 text-sm leading-relaxed text-navy">
                  {item}
                </li>
              ))}
            </ul>
          ...
          {style ? (
            <aside className="border border-border bg-beige p-6">
              ...
              <Link
                ...
                className="mt-5 inline-block border border-navy px-5 py-2.5 text-sm text-navy transition-colors hover:border-gold hover:text-teal"
              >
                Explore {style.name}
              </Link>
            </aside>
...
function ProjectNotFound() {
  return (
    <Section>
      <span className="rule-gold mb-4" aria-hidden="true" />
      <h1 className="font-display text-3xl text-navy">We could not find that project</h1>
      <p className="mt-3 text-sm text-muted-foreground">Browse the full portfolio instead.</p>
      <Link
